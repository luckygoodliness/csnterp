 ...
 */
@Scope("singleton")
@Controller("scm_contract_prepare-after-cancel")
@Transactional

public class ScmContractAfterCancelAction implements IAction {
    ...
    public Map perform(Map inMap) throws BizException, SysException {
        ...
        Integer state = 0;
        if (StringUtil.isNotEmpty(inMap.get("wf_status_closed"))&&(Boolean) inMap.get("wf_status_closed")) {//撤销到第一步
            contractState = 1;//合同编辑
            state = 0;//已保存
        } else {//不是撤销到第一步
            contractState = 3;//合同编辑
            state = 1;//已保存
        }
