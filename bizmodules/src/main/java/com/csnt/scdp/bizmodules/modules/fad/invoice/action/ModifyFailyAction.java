...
@Scope("singleton")
@Controller("failyinvoice-modify")
@Transactional
public class ModifyFailyAction extends CommonModifyAction {
	private static ILogTracer tracer = LogTracerFactory.getInstance(ModifyFailyAction.class);
    ...
    protected void afterAction(BasePojo dtoObj) {
        ...
        if("0".equals(fadInvoiceDto.getState())){//只在提交之前做校验，因为提交后已经本数据已经在占用额度中，并且提交之前不可以改大额度
            ...
            if(StringUtil.isNotEmpty(fadInvoiceDto.getPrmProjectMainId())){
                invoiceManager.checkBudgetIsEnough(true,fadInvoiceDto.getSubjectId(),payCash);
            }else {
                ...
                invoiceManager.checkBudgetIsEnough(false,fadInvoiceDto.getSubjectId(), payCash);
            }
        }
    }
}
