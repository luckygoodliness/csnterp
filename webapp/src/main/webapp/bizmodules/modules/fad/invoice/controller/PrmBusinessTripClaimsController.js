Ext.define("Invoice.controller.PrmBusinessTripClaimsController", {
    extend: 'ErpMvc.controller.ErpAbstractCrudController',
    viewClass: 'Invoice.view.PrmBusinessTripClaimsView',
    ...
    afterModify: function () {
        ...
        var state = me.view.getCmp("fadInvoiceDto->state").value;
        if (state == '1') {
            me.setFieldEditable();
        }
    },
    ...
    pickCashReq: function () {//相关合同弹出窗
        ...
        var prmProjectMainId = me.view.getCmp("fadInvoiceDto->prmProjectMainId").gotValue();
        if (renderPerson == null || renderPerson == '') {
            Scdp.MsgUtil.info("请先选择出差人！");
            return false;
        }
        if (prmProjectMainId == null || prmProjectMainId == '') {
            Scdp.MsgUtil.info("请先选择项目！");
            return false;
        }
...
//
    ...
    updateClearanceMoney: function () {//更新原请款金额、核销总额、报现金额
    ...
    fadInvoiceTravelGridChange: function (a, b) {//交通及住宿费grid改变时
        ...
        this.updateExpensesMoney();//更新报销总金额，更新核准金额
    },
    updateExpensesMoney: function () {//更新报销总金额，更新核准金额
        var me = this;
        var view = me.view;
        ...
        var fadInvoiceTotal = Erp.MathUtil.plusNumber(invoiceTotalValue ,subsidyMoney,0);
        var fadExpensesMoney = Erp.MathUtil.plusNumber(expensesMoney ,subsidyMoney,0);
        view.getCmp("fadInvoiceDto->invoiceTotalValue").sotValue(fadInvoiceTotal);//更新报销总金额
        view.getCmp("fadInvoiceDto->expensesMoney").sotValue(fadExpensesMoney);//更新核准金额
        me.updatePayCash();
    },
    ...
    setFieldEditable: function () {
        ...
        fadCashReqInvoiceGrid.on('beforeedit', function (editor, eventObj) {
            ...
            if (field == 'clearanceMoney' || Scdp.Const.UI_INFO_STATUS_NEW == view.getui) {
                return true;
            } else {
                return false;
            }
